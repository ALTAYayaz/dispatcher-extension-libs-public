...
- It is recommended that all BeYondWMS Database Packages are installed to their own database schema rather than the schema that Dispatcher is installed too
    ...
    - The Following Oracle Grants are required for BEYOND_DEBUG installation (Dispatcher installation schema usually includes these as part of setup)
...
BEGIN
  ...
  BEYOND_DEBUG.print($$PLSQL_UNIT, $$PLSQL_LINE, NULL, 'Found Order:', BEYOND_DEBUG.gc_debugLevelInfo);
  BEYOND_DEBUG.print($$PLSQL_UNIT, $$PLSQL_LINE, 'ORDER_ID', 1001, BEYOND_DEBUG.gc_debugLevelInfo);
  BEYOND_DEBUG.print($$PLSQL_UNIT, $$PLSQL_LINE, 'START_TS', SYSTIMESTAMP, BEYOND_DEBUG.gc_debugLevelInfo);
END;
/
```
...
BEGIN
  ...
  BEYOND_DEBUG.printList($$PLSQL_UNIT, $$PLSQL_LINE, l_loggingList, BEYOND_DEBUG.gc_debugLevelInfo);
END;
/
```
